...
public final class CoreDataManager {
    ...
    public init(dataModelName: String, isStoredInMemoryOnly: Bool) {
    ...
    public func saveAndWait(in context: NSManagedObjectContext) async -> Bool {
    ...
    /// - Parameter context: The managed object context to be saved
    /// - Returns: TRUE if context is saved successfully otherwise FALSE
    public func saveAndWait(in context: NSManagedObjectContext) -> Bool {
    ...
    public func save(_ context: NSManagedObjectContext, completion: ((Bool) -> Void)? = nil) {
        if context != mainManagedObjectContext {
            context.perform {
                var result = true
                defer {
    ...
    public func getMatchingEntities<T: NSManagedObject>(
        _ entity: T.Type,
    ...
    ///   - predicate: Predicate for filtering the data
    ///   - context: managed object context instance
    public func deleteEntities<T: NSManagedObject>(_ entity: T.Type, with predicate: NSPredicate? = nil, in context: NSManagedObjectContext) async {
    ...
    ///   - predicate: Predicate for filtering the data
    ///   - context: managed object context instance
    public func deleteEntities<T: NSManagedObject>(_ entity: T.Type, with predicate: NSPredicate? = nil, in context: NSManagedObjectContext) {
        context.performAndWait {
            let list = self.getMatchingEntities(entity, with: predicate, in: context)
